...
exports.getResult = (req, res, next) => {
  ...
  ResultElection.findAll({ where: { ElectionId: idElection } })
    .then((result) => {
      ...
      for (let x = 0; x < resultElection.length; x++) {
        ...
        let contricantes = resultElection.filter(
          (props) => props.ElectivePositionId == idElectivePosition
        );
        let votesTotal = 0;
        ...
        if (resultElection[x].votes == 0) {
          ...
          resultElection[x].porcentaje = `${
            100 * (resultElection[x].votes / votesTotal)
          }%`;
        }
      }
      ...
      ElectivePosition.findAll({ where: { status: true } })
        .then((result) => {
          const electivePositionActive = result.map((e) => e.dataValues);
          console.log(electivePositionActive);
